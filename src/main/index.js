...
app.whenReady().then(() => {
  ...
  ipcMain.on ('createLivro',function (event, book) {
    LivroModel.create(book).then(() => {
  ...
  ipcMain.handle ('getLivro',async (event, ...args) => {
   return (await LivroModel.findAll()).map((livro) => livro.dataValues);
  });
